...
def load_mnist_dataset(dataset, path):
    ...
    # Create lists for samples, labels, and filenames
    X = []
    y = []
    filenames = []  # ADD THIS
    ...
    for label in labels:
        ...
        for file in os.listdir(os.path.join(path, dataset, label)):
            ...
            X.append(image)
            y.append(label)
            filenames.append(f"{dataset}/{label}/{file}")  # ADD THIS - store the relative path

    # Convert the data to proper numpy arrays and return
    return np.array(X), np.array(y).astype('uint8'), filenames  # ADD filenames to return


# MNIST dataset (train + test)
def create_data_mnist(path):
    # Load both sets separately
    X, y, filenames_train = load_mnist_dataset('train', path)  # ADD filenames
    X_test, y_test, filenames_test = load_mnist_dataset('test', path)  # ADD filenames

    # And return all the data
    return X, y, X_test, y_test, filenames_train, filenames_test  # ADD filenames
...
def test_model():
    ...
    print("="*50)
    print("\n📂 Loading MNIST dataset...")
    X, y, X_test, y_test, filenames_train, filenames_test = create_data_mnist('mnist_png')  # ADD filenames
    X_test_images = X_test.copy()  # Keep original images for display
    X_test = (X_test.reshape(X_test.shape[0], -1).astype(np.float32) - 127.5) / 127.5
    ...
    for i in range(10):
        ...
        print(f"\n{is_correct} Test {i+1}:")
        print(f"   File: {actual_filename}")  # ADD THIS LINE
        print(f"   Predicted: {prediction} (Confidence: {confidence:.2f}%)")
        print(f"   Actual: {test_label}")
        
        # Display image
        display_image = cv2.resize(original_image, (280, 280), interpolation=cv2.INTER_NEAREST)
        
        # Create a larger canvas with black background
        canvas = np.zeros((420, 280), dtype=np.uint8)  # Made taller for filename
        ...
        # Show filename (might be long, so use smaller font)
        filename_short = actual_filename.split('/')[-1]  # Just the filename, not full path
